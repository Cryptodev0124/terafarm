...
export const useCake = () => {
  ...
  return useContract((GTOKEN[chainId].address) ?? GTOKEN[ChainId.SEPOLIA].address, erc20Abi)
}
...
export const useLaunchpad = (address: Address) => {
  const { chainId } = useActiveChainId()
  const { data: signer } = useWalletClient()
  return useMemo(() => getLaunchpadContract(address, signer, chainId), [address, signer, chainId])
}

export const useContributionProgram = () => {
  const { chainId } = useActiveChainId()
  const { data: signer } = useWalletClient()
  return useMemo(() => getContributionContract(signer, chainId), [signer, chainId])
}

export const useSmartRouter = () => {
  const { chainId } = useActiveChainId()
  const { data: signer } = useWalletClient()
  return useMemo(() => getSmartRouterContract(signer, chainId), [signer, chainId])
}