...
export const fetchChartData = async (
  ...
  while (!allFound && !error) {
    ...
    skip += 1000
    allFound = (data && data?.length < 1000) || skip > 2000
    error = fetchError
    if (data) {
...
export async function getPairTokenMap(poolAddresses: string[], chainId: ChainId) {
  const client = publicClient({chainId})
  let rawPairTokenResults: any[] = []
  const calls = poolAddresses
    .map((poolAddress) => {
      return [
        { address: poolAddress as `0x${string}`, functionName: 'token0', abi: lpTokenABI },
        { address: poolAddress as `0x${string}`, functionName: 'token1', abi: lpTokenABI },
      ]
    })
    .flat()
  try {
  ...
  //   .map((pairToken) => {
  //     return [
  //       { address: pairToken, functionName: 'name', abi: erc20Abi },
  //       { address: pairToken, functionName: 'symbol', abi: erc20Abi },
  //     ]
  //   })
  //   .flat()
  try {
